...
from datastore_version_manager.domain import (
    pending_operations, metadata_all, data_versions, datastore_versions
)
...
def bump_version(description: str):
    ...
    bumped_data_structures, new_version = (
        datastore_versions.bump_datastore_versions(
            description, pending_ops, release_time, previous_version
        )
    )
    metadata_all.create_new_version(
        bumped_data_structures, previous_version, new_version
    )
    ...
    data_versions.create_new_version(
        bumped_data_structures, previous_version, new_version
    )
    datastore.remove_archived_pending_operations(
        pre_bump_pending_operations, new_version
    )


def _change_metadata_file_names(bumped_data_structures: dict,
